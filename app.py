...
if check_password():
    ...
    # --- IDENTIDADE ATUALIZADA: ARA + 30 PALAVRAS + EMOJIS + HASHTAGS FIXAS ---
    identidade_igreja = """
    ...
    HASHTAGS OBRIGATÓRIAS: Sempre finalize a legenda com as hashtags: #isosedcosmopolis #isosed
    """
    ...
    with aba_feed:
        ...
        versiculo_base = st.text_input("📖 Versículo Base", placeholder="Ex: Filipenses 4:13")
        ...
        with col2:
            tema_feed = st.text_area("Tema do Post", placeholder="Ex: Chamada para o culto de libertação...")
            instrucoes = st.text_input("Direcionamento Extra", placeholder="Ex: Mencionar que teremos Santa Ceia...")
        
        if st.button("✨ Gerar Legenda ARA"):
            if tema_feed and versiculo_base:
                with st.spinner('Gerando legenda com hashtags...'):
                    prompt_f = f"{identidade_igreja} \nVERSÍCULO: {versiculo_base}. \nCrie uma legenda detalhada para {plataforma} (+30 palavras) com emojis e as hashtags fixas. Tema: {tema_feed}. Tom: {tom_de_voz}. Obs: {instrucoes}. Use estrutura AIDA."
    ...
    with aba_stories:
        ...
        versiculo_st = st.text_input("📖 Versículo para Stories", placeholder="Ex: Josué 1:9")
        tema_st = st.text_area("Tema dos Stories", placeholder="Ex: Bom dia abençoado...")
        
        if st.button("💡 Gerar Sequência"):
            if tema_st and versiculo_st:
                with st.spinner('Criando roteiro...'):
                    # Stories geralmente não usam hashtags fixas no texto, mas se quiser, posso adicionar!
